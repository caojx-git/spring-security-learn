...
/**
 * 重写 JdbcConnectionRepository，因为MySql 8.0.19中不支持 UserConnection 表中的rank字段，我把UserConnection表字段修改为rank_1
 *
 * @author caojx
 * @version $Id: MyJdbcConnectionRepository.java,v 1.0 2020/3/9 7:10 下午 caojx
 * @date 2020/3/9 7:10 下午
 */
public class MyJdbcConnectionRepository implements ConnectionRepository {
    ...
    private final MyJdbcConnectionRepository.ServiceProviderConnectionMapper connectionMapper = new MyJdbcConnectionRepository.ServiceProviderConnectionMapper();
    ...
    public MultiValueMap<String, Connection<?>> findAllConnections() {
        List<Connection<?>> resultList = this.jdbcTemplate.query(this.selectFromUserConnection() + " where userId = ? order by providerId, rank_1", this.connectionMapper, new Object[]{this.userId});
    ...
    public List<Connection<?>> findConnections(String providerId) {
        return this.jdbcTemplate.query(this.selectFromUserConnection() + " where userId = ? and providerId = ? order by rank_1", this.connectionMapper, new Object[]{this.userId, providerId});
    }
    ...
    public MultiValueMap<String, Connection<?>> findConnectionsToUsers(MultiValueMap<String, String> providerUsers) {
        if (providerUsers != null && !providerUsers.isEmpty()) {
            ...
            List<Connection<?>> resultList = (new NamedParameterJdbcTemplate(this.jdbcTemplate)).query(this.selectFromUserConnection() + " where userId = :userId and " + providerUsersCriteriaSql + " order by providerId, rank_1", parameters, this.connectionMapper);
    ...
    public void addConnection(Connection<?> connection) {
        try {
            ...
            int rank_1 = (Integer) this.jdbcTemplate.queryForObject("select coalesce(max(rank_1) + 1, 1) as rank_1 from " + this.tablePrefix + "UserConnection where userId = ? and providerId = ?", new Object[]{this.userId, data.getProviderId()}, Integer.class);
            this.jdbcTemplate.update("insert into " + this.tablePrefix + "UserConnection (userId, providerId, providerUserId, rank_1, displayName, profileUrl, imageUrl, accessToken, secret, refreshToken, expireTime) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", new Object[]{this.userId, data.getProviderId(), data.getProviderUserId(), rank_1, data.getDisplayName(), data.getProfileUrl(), data.getImageUrl(), this.encrypt(data.getAccessToken()), this.encrypt(data.getSecret()), this.encrypt(data.getRefreshToken()), data.getExpireTime()});
    ...
    private Connection<?> findPrimaryConnection(String providerId) {
        List<Connection<?>> connections = this.jdbcTemplate.query(this.selectFromUserConnection() + " where userId = ? and providerId = ? order by rank_1", this.connectionMapper, new Object[]{this.userId, providerId});
